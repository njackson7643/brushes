...
top_bound = int((chain_len+init_substr_sep+1)*poly_bond_len/grid_disc)
...
if branch_choice == "no":
    ...
    for i in range(offset,Lx,chain_sep_1D):
        ...
        for j in range(offset,Ly,chain_sep_1D):
            seq_count = 0
            for k in range(top_bound-bond_size,top_bound-poly_len-1,-bond_size):
                num_bond += 1
                sim_grid[i,j,k] = str(chain_list[seq_count%seq_len])
                seq_count += 1
                if sim_grid[i,j,k] == "P":
...
c_count = 0
d_count = 0
#PROBABLY NEED TO CHANGE SALT CONCENTRATIONS SO THAT I CAN USE FIX DEFORM TO ADJUST THE BOX SIZE
...
while len(ctr_list) > 0:
    ...
    zrand = random.randint(bond_size,int(top_bound-1))
