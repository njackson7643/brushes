...
plt.figure(1)
plt.subplot(221)
plt.plot(z_arr,z_dist,color='black',linewidth=2)
plt.xlim(zmin,zmax)
plt.ylim(0,max(z_dist))
plt.title('Polymer monomer density distribution')
...
plt.title('Side face view of polymer density distribution.  Color map = probability.')
plt.xlabel('averaged x/y dimension')
...
for i in range(len(z_mean_arr)):
    if z_mean_arr[i] > 0.0:
        plt.plot(z_arr,z_dist_arr[i],color=color_list[i],linewidth=2,label=legend_list[i])
        plt.xlim(zmin,zmax)
        plt.ylim(0,max(z_dist_arr[i]))
    plt.title('Counter ion density distribution on z-axis')
...
for i in range(len(z_mean_arr)):
    if z_mean_arr[i] > 0.0:
        ...
        plt.xlim(zmin,zmax)
        plt.ylim(0,max(z_dist_arr[i]))
    plt.title('Salt ion density distribution on z-axis')
    plt.xlabel('z direction')
    plt.ylabel('probability')
    plt.legend()
