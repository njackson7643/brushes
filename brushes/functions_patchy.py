...
#Write neutral patchy atoms
    if e_count > 0:
        mol_count += 1
        typ_count += 1
        for i in range(1,e_count+1,1):
            curr = e_dict[i].split(',')
            wfile.write(str(i+S_count+Z_count+P_count+N_count+p_count+n_count+a_count+b_count+c_count+d_count)+" "+str(mol_count)+" "+str(typ_count+1)+"\t"+str(chg_dict[atom_type_list[typ_count]])+"\t"+str(float(curr[0])*grid_disc)+"\t"+str(float(curr[1])*grid_disc)+"\t"+str(float(curr[2])*grid_disc)+"\n")
#Write negative patchy atoms
    if f_count > 0:
        mol_count += 1
        typ_count += 1
        for i in range(1,f_count+1,1):
            curr = f_dict[i].split(',')
            wfile.write(str(i+S_count+Z_count+P_count+N_count+p_count+n_count+a_count+b_count+c_count+d_count+e_count)+" "+str(mol_count)+" "+str(typ_count+1)+"\t"+str(chg_dict[atom_type_list[typ_count]])+"\t"+str(float(curr[0])*grid_disc)+"\t"+str(float(curr[1])*grid_disc)+"\t"+str(float(curr[2])*grid_disc)+"\n")
#Write positive patchy atoms
    if g_count > 0:
        mol_count += 1
        typ_count += 1
        for i in range(1,g_count+1,1):
            curr = g_dict[i].split(',')
            wfile.write(str(i+S_count+Z_count+P_count+N_count+p_count+n_count+a_count+b_count+c_count+d_count+e_count+f_count)+" "+str(mol_count)+" "+str(typ_count+1)+"\t"+str(chg_dict[atom_type_list[typ_count]])+"\t"+str(float(curr[0])*grid_disc)+"\t"+str(float(curr[1])*grid_disc)+"\t"+str(float(curr[2])*grid_disc)+"\n")
#Write BONDS section
    wfile.write("\n")
    wfile.write("Bonds\n\n")
#    print rev_bond_dict
    for i in range(1,b_typ_1_len+1,1):
        curr = rev_bond_dict[i].split(',')
        wfile.write(str(i)+"\t"+str(1)+"\t"+str(curr[0])+"\t"+str(curr[1])+"\n")
    for i in range(b_typ_1_len+1,b_typ_1_len+b_typ_2_len+1,1):
        curr = rev_bond_dict[i].split(',')
        wfile.write(str(i)+"\t"+str(2)+"\t"+str(curr[0])+"\t"+str(curr[1])+"\n")
    wfile.write("\n")
    wfile.write("Angles\n\n")
...
#Set short range LJ cutoff to 2.5 and use shift.  Electrostatics cutoff set to > 2x LJ cutoff
    ...
    wfile.write("bond_style \t hybrid\t harmonic fene \n")
...
def write_settings(filename,LJ_dict,atom_type_list,lin_angle_dict,per_angle_dict,angle_coeff_dict,poly_bond_len,poly_bond_k,FENE_bond_len,patch_bond_k,patch_FENE_max):
    wfile = open(filename,'w')
    ...
    wfile.write("bond_coeff 1 fene "+str(poly_bond_k)+" "+str(FENE_bond_len)+" 0.1 1.0\n")
    wfile.write("bond_coeff 2 harmonic "+str(patch_bond_k)+" "+str(patch_FENE_max)+"\n\n")
...
def write_infile(filename,tstep,equil_steps,sample_steps,temp,substr_len,atom_type_list,dump_int,dielectric,thermo_step,Lx,Ly,top_bound,grid_disc):
    ...
    salt_typ = 0
    patchy_typ = 3
    for item in atom_type_list:
        if item == 'N' or item == 'Z' or item == 'P':
            poly_typ += 1
        elif item == 'a' or item == 'b' or item == 'c' or item == 'd':
            salt_typ += 1
        else:
            continue
    #WRITE POLYMER GROUPS
    if poly_typ == 1:
        wfile.write("group polymers type "+str(type_count)+"\n")
    ...
    type_count += 1
    if salt_typ != 0:
        wfile.write("group salt type "+str(type_count)+":"+str(type_count+salt_typ)+"\n")
        type_count = type_count + salt_typ + 1
    #WRITE PATCHY GROUPS
    wfile.write("group patchy type "+str(type_count)+":"+str(type_count+3)+"\n")
...
#        wfile.write("group ctr_ions type 3:4\n")
#        wfile.write("group patchy type 5:7\n")
